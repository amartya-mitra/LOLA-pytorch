...
r1 = Variable(torch.Tensor([-1, 1, 1, -1]).type(dtype))
r2 = Variable(torch.Tensor([1, -1, -1, 1]).type(dtype))
I = Variable(torch.eye(4).type(dtype))

gamma = Variable(torch.Tensor([0.9]).type(dtype))
delta = Variable(torch.Tensor([0.1]).type(dtype))
eta = Variable(torch.Tensor([1]).type(dtype))
...
for epoch in range(1000):
	...
	#y1.data += (delta*dV1[0] + delta*eta*torch.matmul(dV1[1].t(),d2V2Tensor).t()).data
	#y2.data += (delta*dV2[1] + delta*eta*torch.matmul(dV2[0].t(),d2V1Tensor).t()).data
